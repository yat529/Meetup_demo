<template>
  <v-container px-0 py-0 class="friend-list">
    <!-- <v-layout row> -->
      <!-- toolbar -->
      <v-layout justify-end class="friend-list-toolbar py-2">
        <v-btn small round flat class="primary--text" :class="EditMode ? 'btn--active' : ''" @click="toggleEditMode">
          ...
          {{ isMobile ? '编辑列表' : '编辑朋友列表' }}
        </v-btn>
      </v-layout>
      <v-container class="placeholder mt-4" :class="isMobile ? 'height-fix' : ''" v-if="!friends.length">
        <v-layout justify-center align-center fill-height>
          <v-btn flat large color="primary">目前还没有好友</v-btn>
        </v-layout>
      </v-container>
      <v-container class="mt-4" v-else>
        <UserCards :users="friends" :isMobile="isMobile"
