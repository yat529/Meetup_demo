<template>
  <v-container fluid px-0 py-0 class="meetup-page-wrapper" v-if="item.uid">
    <!-- float panel -->
    <div class="float-pan" v-show="showFloatPan" v-if="!isMobile">
    ...
    <!-- modal panel -->
    <v-layout row justify-center v-else-if="isMobile">
      <v-dialog v-model="showFloatPan" persistent max-width="400">
        <div class="float-pan re-position" v-show="showFloatPan">
          <div class="minimize" @click="showFloatPan = false">
            <i class="fas fa-minus-circle"></i>
          </div>
          <div class="date-info" :class="EditMode ? 'highlight-border' : ''">
    ...
    <!-- header -->
    <v-layout row class="header" :class="isMobile ? 'onMobile' : ''" v-if="item.uid">
      <v-container px-4>
        ...
        <v-layout class="meta-bar px-0 py-0" row wrap>
          <v-flex class="organizer">
          ...
          <v-flex>
            <v-layout align-center justify-end v-show="!showFloatPan">
              <div @click="showFloatPan = true">
                <calendarIcon :date="dateObj"></calendarIcon>
              </div>
              <div style="padding-right: 15px" v-show="!isMobile"></div>
              <v-btn round color="primary" v-show="!isMobile" @click="showFloatPan = true">
    ...
    <v-layout row wrap>
      <v-flex xs12 class="meetup">
        <v-card flat>
          <!-- image -->
          <v-card-media :src="getImgUrl(item)" height="500px">
          </v-card-media>
          <!-- card body -->
          <v-container>
            <!-- members -->
            <v-layout class="seats px-2 py-2" :class="!isMobile ? 'mb-5' : ''" row wrap align-center>
              ...
              <v-flex xs12 sm9 offset-sm1 :class="EditMode ? 'highlight-border' : ''">
                <Seats :class="isMobile ? 'center' : ''"
                :group="groupMembers" :size="item.size" :meetupKey="item.key"
                :showRedirectAndAddFriend="!EditMode"
                :showRemoveMember="EditMode"></Seats>
              </v-flex>
            </v-layout>
            <!-- description -->
            <v-layout class="orgnizer px-2 py-2" :class="!isMobile ? 'mb-5' : ''" row wrap align-center>
              <v-flex xs12 sm2 class="text-xs-center my-4">
            ...
            <v-layout class="orgnizer px-2 py-2" :class="!isMobile ? 'mb-5' : ''" row wrap align-center>
              ...
              <v-flex xs12 sm9 offset-sm1>
                <v-layout class="grid-wrapper" row wrap>
                  <v-flex class="px-2 py-2" xs6 lg3
  ...
  computed: {
    ...
    addressURL () {
      ...
      if (location) {
        ...
        url = `https://www.google.com/maps/search/?api=1&query=${lat},${lng}&${name}`
      }
      return url
    },
    vancancyCount () {
